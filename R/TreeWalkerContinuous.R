	...
	BranchWalker <- function(slon = 0, slat = 0, step_times, step_size, edge_start_time, edge_end_time, steplengthmean = steplengthmean, steplengthsd = steplengthsd) {
		...
		if(start_step <= end_step) {
			...
			# Case if branch starts before a step value:
				...
				first_step <- EndPoint(longitude = slon, latitude = slat, bearing = stats::runif(1, 0, 360), distance = step_fraction * abs(rnorm(1, steplengthmean, steplengthsd)), EarthRad = EarthRad)
			...
			for(i in start_step:end_step) {
				...
				# Make next step:
				next_step <- EndPoint(longitude = start_lon, latitude = start_lat, bearing = stats::runif(1, 0, 360), distance = abs(rnorm(1, steplengthmean, steplengthsd)), EarthRad = EarthRad)
			...
			# Case if branch ends after a step value:
				...
				last_step <- EndPoint(longitude = step_matrix[2, as.character(end_step)], latitude = step_matrix[3, as.character(end_step)], bearing = stats::runif(1, 0, 360), distance = step_fraction * abs(rnorm(1, steplengthmean, steplengthsd)), EarthRad = EarthRad)
		...
		# Case if no steps on branch:
			...
			branch_step <- EndPoint(longitude = slon, latitude = slat, bearing = stats::runif(1, 0, 360), distance = step_fraction * abs(rnorm(1, steplengthmean, steplengthsd)), EarthRad = EarthRad)
